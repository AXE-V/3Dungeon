...
export const useSignEvents = (_id: any, path: string) => {
  ...
  useEffect(() => {  
    signR._id !== _id ? (
      styleController(null, {currentTgNodes: ((signRef.current as unknown) as HTMLInputElement)
  ...
  return {
    ...
    onClick<E extends SyntheticEvent<EventTarget>>(e: E) {
      dispath(setActiveSign({_id: _id}))
      signR._id !== _id ? (
        styleController(e, {command: 'add', style: { fill: '#fff', opacity: .75}})
      ) : void 0         
      navigate(path)
    },

    onMouseLeave<E extends SyntheticEvent<EventTarget>>(e: E) {
      signR._id !== _id ? styleController(e, {command: 'remove'}) : void 0
    },
