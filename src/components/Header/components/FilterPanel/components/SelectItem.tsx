...
import { SVGComponent, SelectInter } from "../../../../../interfaces/SVGComponent";
...
export const SelectItem: FC<SVGComponent & SelectInter> = ({children, style, _id, selectedItem, setSelectedItem, setIsOpen}) => {
  ...
  useEffect(() => {
    // selectedItem === _id ? 
    // childStyleController(((itemRef.current as unknown) as HTMLElement)
    //   .childNodes[1].childNodes, 'add', 
    //   {fill: '#c6b63f'}) : 
    // childStyleController(((itemRef.current as unknown) as HTMLElement)
    //   .childNodes[1].childNodes, 'remove');
  }, [selectedItem])

  function onMouseOver<E extends SyntheticEvent<EventTarget>>(e: E) {
    // selectedItem !== _id ? 
    //   childStyleController((e.currentTarget as HTMLElement)
    //     .childNodes[1].childNodes, 'add', 
    //     {fill: '#1e1e1e'}) : void 0;
  }
  ...
  function onMouseLeave<E extends SyntheticEvent<EventTarget>>(e: E) {
    // selectedItem !== _id ? 
    // childStyleController((e.currentTarget as HTMLElement)
    //   .childNodes[1].childNodes, 'remove') : void 0;
  }
